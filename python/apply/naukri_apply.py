...
sys.path.insert(0, str(ROOT_DIR / "python" / "shared"))

from playwright.sync_api import TimeoutError, sync_playwright

from shared_answers import capture_visible_answers, fill_known_answers, load_answers

AUTH_FILE = ROOT_DIR / "auth" / "naukri_auth.json"
HISTORY_FILE = ROOT_DIR / "data" / "naukri_application_history.json"
ATTEMPTS_FILE = ROOT_DIR / "data" / "naukri_application_attempts.jsonl"
...
def parse_posted_age_hours(text: str):
    """Return estimated hours since posted for common human-readable strings.
    Returns a float number of hours, or None if it cannot be parsed.
    """
    if not text:
        return None
    t = text.strip().lower()
    # common quick variants
    if "just" in t or "now" in t or "minutes" in t or "mins" in t or "min" in t:
        m = re.search(r"(\d+)\s*min", t)
        if m:
            return float(int(m.group(1))) / 60.0
        return 0.1

    # hours
    m = re.search(r"(\d+)\s*(?:hour|hr|hrs)", t)
    if m:
    ...
    if m:
        return float(int(m.group(1))) * 24.0

    # generic patterns like 'posted today' or 'today'
    if "today" in t:
        # ambiguous; assume within 24 hours but not precise
        return 12.0

    # ISO or explicit date e.g. '04 Jun 2026' or '4 Jun'
    try:
        # try a couple of common date formats
        for fmt in ("%d %b %Y", "%d %b %y", "%d %b", "%Y-%m-%d"):
            try:
...
def find_posted_text_from_card(job_card):
    selectors = [
        ".date",
        ".postedOn",
        ".posted-on",
        ".job-posted",
        "time",
        ".jobTupleFooter .meta, .job-meta, .jd-stats .info",
    ]
    for selector in selectors:
        try:
            loc = job_card.locator(selector)
            if loc.count():
    ...
    with sync_playwright() as playwright:
        ...
        for keyword in config["keywords"]:
            for location in config["locations"]:
                ...
                # Collect posted-age for each visible job card so we can process newest first
                card_info = []
                for i in range(total_jobs):
                    try:
                        card = job_cards.nth(i)
                        posted_text = find_posted_text_from_card(card)
                        hours = parse_posted_age_hours(posted_text)
                        card_info.append((i, hours))
                    except Exception:
                        card_info.append((i, None))

                # sort: newest (smallest hours) first; unknown ages go last
                sorted_indices = [idx for idx, _ in sorted(card_info, key=lambda t: (t[1] is None, t[1] if t[1] is not None else 1e9))]
                log(f"Processing {len(sorted_indices)} job cards (sorted by age, newest first)")
                ...
                for index in sorted_indices:
                    ...
                    try:
                        ...
                        # Filter by posted age when available on the job card
                        try:
                            posted_text = find_posted_text_from_card(job_card)
                            hours = parse_posted_age_hours(posted_text)
                            if hours is not None and hours > config.get("posted_within_hours", 0):
                                log(f"⏭ Skipped (posted ~{hours:.1f}h ago) — older than {config.get('posted_within_hours')}h")
                                continue
                        except Exception:
                            # if parsing fails, fall back to including the job
                            pass
